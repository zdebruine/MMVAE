...
class CLVAE(VAE):
    ...
    def after_reparameterize(self, z: torch.Tensor, metadata: pd.DataFrame) -> torch.Tensor:
        """
        Modify the latent variable after reparameterization by applying conditional layers.

        If conditional layers are defined, they will be applied to the latent variable `z`
        ...
        Returns:
            torch.Tensor: Processed latent variable after applying conditionals, if any.
        """
        if self.conditionals:
            return self.conditionals(z, metadata)
        return z  # Return the unmodified latent variable if no conditionals are present