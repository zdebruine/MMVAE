...
def start_server(source_dir, output_dir, port: Union[Literal['auto'], int] = 8000):
    """Start an HTTP server serving files from the specified directory."""

    try:
        handler = QuietHTTPRequestHandler
        httpd = HTTPServer(("", port), lambda *args, **kwargs: handler(*args, directory=output_dir, **kwargs))
        ...
        if e.errno == 98:
            ...
            exit(1)
        raise e
    # Run the server in a separate thread to allow for rebuilds in the main thread
    ...
    try:
        while True:
            command = input("Type '(R|build)' to rebuild pdoc documentation and (C|clean) to remove all: ").lower()
