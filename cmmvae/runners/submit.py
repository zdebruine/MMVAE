...
def parse(v):
    if isinstance(v, list):
        value_strs = (
            f'"{str(elem)}"' if isinstance(elem, str) else str(elem)
            for elem in v
        )
...
def parse_tracked_command(command_key, command):
    ...
    if isinstance(command, dict) and len(command) == 1 \
            and isinstance(list(command.values())[0], list):
        for name, val in command.items():
            name = name.replace("{value}", str(val))
            commands.append((command_key, val, name))
...
class Experimenter:
    ...
    def build_job_commands(self):
        ...
        for job in self.jobs:
            ...
            commands = [
                f"{key_to_command(command)} {value}"
                for command, value, _ in job
            ]
            train_command_string = f"{self.subcommand} {' '.join(commands)}"
            run_config['train_command'] = train_command_string
    ...
    def setup_experiments(self):
        command_items = self.config['train_command'].items()
        self.subcommand, train_commands = next(iter(command_items))
        command_pairs = self.parse_command_key_combinations(train_commands)
        from itertools import product
        self.jobs = list(product(*command_pairs))

    def parse_command_key_combinations(self, commands: dict):
        combinations = []
        for command_key, command in commands.items():
            if isinstance(command, dict):
                if 'track' in command:
                    command = command['track']
                    tracked_commands = parse_tracked_command(
                        command_key, command)
    ...
    def validate_experiments(self):
        ...
        if not hasattr(self, 'jobs'):
            raise RuntimeError(
                "setup_experiments failed: attr 'jobs' not available")

        if len(self.jobs) > self.max_job_limit:
            raise RuntimeError(
                f"""
                Number of jobs configured to run '{len(self.jobs)}
                exceeds 'max_job_limit' of {self.max_job_limit}.
                Look over the job configuration and if number of jobs
                is correct consider raising 'max_job_limit'.
                """)
        elif self.max_job_limit - 1 <= len(self.jobs):
            import warnings
            warnings.warn(
                f"""
                Number of jobs is close to limit configured meaning
                the configuration file supplied may not be performing
                as expected, otherwise you can ignore this message.
...
def parse_kwargs(ctx, param, value):
    ...
    for item in value:
        try:
            ...
            msg = f'Invalid format for {param.name}. Expected: key=value'
...
@experiment.command()
@click.option("--config_file", type=str, default="experiments.yaml",
              show_default=True, help="Path to configuration file.")
@click.option('--config', multiple=True, callback=parse_kwargs,
              help="Configuration options as key=value pairs")
@click.option("-m", "--max_job_limit", type=int, default=3, show_default=True,
              help="Max number of jobs capable of outputting without failure.")
@click.option("-t", "--timestamp", is_flag=True,
              help="Added timestamp to end of run name.")
@click.option("-p", "--preview", is_flag=True,
              help="Do not run subprocess, only preview job configurations.")
def submit(**kwargs):
    """
    ...
    Args:
        ...
        preview (bool): Preview job configurations without running them.
    """
