...
class Model(nn.Module):
    ...
    def __init__(self, shared_vae: M.VAE, realism_bc: Discriminator):
        super().__init__()
...
def configure_model(hparams) -> Model:
    ...
    def build_seq(_hparams: dict, name = None):
        ...
        for layer in _hparams.keys():
            if 'BatchNorm1d' in layer:
                num_features, eps = _hparams[layer]['num_features'], _hparams[layer]['eps']
                sequence = (*sequence, nn.BatchNorm1d(num_features, eps))
            else:
