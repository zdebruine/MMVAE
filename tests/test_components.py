...
def test_conditional_layer_initialization():
    conditions_path = (
        f"{os.getcwd()}/src/cmmvae/data/conditional_layers/unique_assays.csv"
    )
    unique_conditions = len(pd.read_csv(conditions_path, header=None))
    ...
    assert len(layer.conditions) == unique_conditions
