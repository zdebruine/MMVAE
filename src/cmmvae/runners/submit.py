...
def parse(v):
    if isinstance(v, list):
        value_strs = (
            f'"{str(elem)}"' if isinstance(elem, str) else str(elem) for elem in v
        )
...
class Experimenter:
    ...
    def build_job_commands(self):
        ...
        for job in self.jobs:
            ...
            commands = [
                f"{key_to_command(command)} {value}" for command, value, _ in job
            ]
            ...
            config_args = [f"{key}={value}" for key, value in run_config.items()]
...
@click.option(
    "-t", "--timestamp", is_flag=True, help="Added timestamp to end of run name."
)
@click.option(
    "-p",
    "--preview",
