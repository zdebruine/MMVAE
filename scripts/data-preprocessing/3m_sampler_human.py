...
def load_data(n):
    ...
    with open(
        f"/active/debruinz_project/human_data/python_data/chunk{n}_metadata.csv", "r"
    ) as metadata_file:
        metadata_reader = csv.reader(metadata_file)
        metadata = list(metadata_reader)
    return (
        sp.load_npz(
            f"/active/debruinz_project/human_data/python_data/human_chunk_{n}.npz"
        ),
        metadata,
    )
...
def write_data(chunk, metadata, n):
    ...
    sp.save_npz(
        f"/active/debruinz_project/CellCensus_3M/3m_human_chunk_{n}.npz",
        sp.vstack(chunk),
    )
    with open(
        f"/active/debruinz_project/CellCensus_3M/3m_human_metadata_{n}.csv", "w"
    ...
    for i in range(1, 101):
        ...
        for idx in indicies:
            ...
            metadata_slices.append(
                metadata[idx + 1]
            )  # Adjusting for t he column header row in the metadata
