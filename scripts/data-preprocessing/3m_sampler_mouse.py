...
def load_data(n):
    ...
    with open(
        f"/active/debruinz_project/mouse_data/python_data/mouse_metadata_{n}.csv", "r"
    ) as metadata_file:
        metadata_reader = csv.reader(metadata_file)
        metadata = list(metadata_reader)
    return (
        sp.load_npz(
            f"/active/debruinz_project/mouse_data/python_data/mouse_chunk_{n}.npz"
        ),
        metadata,
    )
...
def write_data(chunk, metadata, n):
    ...
    sp.save_npz(
        f"/active/debruinz_project/CellCensus_3M/3m_mouse_chunk_{n}.npz",
        sp.vstack(chunk),
    )
    with open(
        f"/active/debruinz_project/CellCensus_3M/3m_mouse_metadata_{n}.csv", "w"
    ) as subset_metadata_file:
    ...
    for i in range(1, 101):
        ...
        for idx in indicies:
            ...
            metadata_slices.append(
                metadata[idx + 1]
            )  # Adjusting for the column header row in the metadata
